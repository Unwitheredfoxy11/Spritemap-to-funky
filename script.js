...
let animData   = null;        // Animation.json (obj)
let lastZipUrl = null;

const pngInput      = document.getElementById('pngInput');
const atlasJsonInput= document.getElementById('jsonInput'); // spritemap JSON
const animJsonInput = document.getElementById('animInput'); // Animation.json input
...
animJsonInput.addEventListener('change', async e=>{
  const f = e.target.files[0];
  if(!f) return setStatus('No seleccionaste el Animation.json.');
  try{
...
// Conectar botón
// =======================
convertirBtn.addEventListener('click', exportFramesFromAnimationToZip);
